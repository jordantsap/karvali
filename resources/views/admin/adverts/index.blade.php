                                    ...
                                    @foreach ($adverts as $advert)
                                        ...
                                        <tr>
                                            ...
                                            <td><img width="150px" height="150px"
                                                     src="{{asset('images/adverts/'.$advert->banner)}}"
                                                     alt="{{$advert->title}}"></td>
                                            <td>
                                                @can ('update_adverts', App\Advert::class)
                                                    <a class="btn btn-primary"
                                                       href="{{route('adverts.edit', $advert->id)}}">Edit</a> -
                                                @endcan
                                                @can ('view_adverts', App\Advert::class)
                                                    <a class="btn btn-primary"
                                                       href="{{route('adverts.show', $advert->id)}}">View</a>
                                                @endcan
                                                @can ('delete_adverts', \App\Models\unused\Advert::class)
