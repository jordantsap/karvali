...
<div class="content-wrapper">
  ...
  <section class="content">
    <div class="box">
      <form action="{{ route('owner.clubevents.store') }}" method="post" role="form" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="box-body">
          ...
          <div class="row">
            <div class="col-xs-2 form-group">
              ...
              <div class="col-xs-2 form-group{{ $errors->has('group_type') ? ' has-error' : '' }}">
                  <label for="group_type">{{__("form.categorytype")}}</label>
                  @if ($errors->has('group_type'))
                      <strong class="text-danger">{{ $errors->first('group_type') }}</strong>
                  @endif
                  <div class="">
                      <select id="group_type" value="{{ old('group_type') }}"
                              name="group_type" class="form-control" required>
                          <option value="{{ old('group_type') }}">{{__('form.pleaseselect')}}</option>
                          @foreach($grouptypes as $venue)
                              <option
                                  value="{{ $venue->id }} {{ old('group_type') }}" {{old('group_type')?"selected":""}}>{{ $venue->title }}</option>
                          @endforeach
                      </select>
                  </div>
              </div>
              <div class="col-xs-2 form-group{{ $errors->has('group_id') ? ' has-error' : '' }}">
                  <label for="group_id">{{__("form.group")}}</label>
                  @if ($errors->has('group_id'))
                      <strong class="text-danger">{{ $errors->first('group_id') }}</strong>
                  @endif
                  <div class="">
                      <select id="group_id" value="{{ old('group_id') }}"
                              name="group_id" class="form-control" required>
                          <option value="{{ old('group_id') }}">{{__('form.pleaseselect')}}</option>
                          @foreach($groups as $venue)
                              <option
                                  value="{{ $venue->id }} {{ old('group_id') }}" {{old('group_id')?"selected":""}}>{{ $venue->title }}</option>
                          @endforeach
                      </select>
                  </div>
              </div>
              <div class="col-xs-2 form-group{{ $errors->has('amenity_id') ? ' has-error' : '' }}">
                  <label for="amenity_id">{{__("form.amenity")}}</label>
