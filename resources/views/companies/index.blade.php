       ...
       <nav class="navbar">
         <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#companytype-collapse"
          ...
          @foreach($companytypes as $companytype)
              <li>
                <a href="{{ route('companies-category', $companytype->slug)}}" class="">{{ $companytype->name }}&nbsp<span class="badge">{{$companytype->companies->where('active',1)->count()}}</span>
              </a>
            </li>
            @endforeach
            <li><script>
         ...
         @if(count($companies) > 0)
           ...
           <div class="card h-100">
             <a href="{{ route('company',$company->slug) }}">
               <img class="img-responsive img-fluid rounded" style="width:100%;height:150px;" src="{{ asset('images/companies/'.$company->logo) }}" alt="{{ $company->title }}">
             </a>
           </div>
             <div class="card-body text-center">
               ...
               <div class="row" id="likecomment">
                 <div class="col-xs-6 text-center">
                   <i class="fa fa-3x fa-thumbs-up"></i><span class="badge">{{$company->likes->count()}}</span>
                 </div>
                 <div class="col-xs-6 text-center">
                   <i class="fa fa-3x fa-comment"></i><span class="badge">{{$company->comments->count()}}</span>
                 </div>
               </div>
               <div class="row">
                 <div class="col-xs-12">
                   <h3><b>{{ __('page.category') }}</b> <a href="{{ route('companies-category', $company->category->slug)}}">{{$company->category->name}}</a></h3>
                 </div>
               </div>
             </div>
         </div>
